...
This needs **two pushes** because the sound file has to sit inside the native Android project,
which only gets created the first time the workflow runs.

1. Push this updated code as normal (see steps above). The workflow will now automatically
   generate the `android/` folder **and commit it into your repository** the first time it runs
   (you'll see a new commit appear, e.g. "Add generated android project (auto)").
2. Once that commit shows up, open your repo and navigate to:
   `android/app/src/main/res/raw/`
   (create the `raw` folder yourself if it isn't there yet — GitHub lets you type a full path
   like `android/app/src/main/res/raw/alarm_tone.wav` when uploading, and it creates the folders for you).
3. Upload your tone file there, named **exactly** `alarm_tone.wav`
   (Android resource names must be lowercase letters, numbers, and underscores only — no spaces,
   dashes, or capital letters, and no other dots besides the extension).
   - If your tone is an mp3, either rename the file's extension check works too (Android accepts
     .mp3 and .ogg as well) — just make sure the filename here matches the `CUSTOM_SOUND` value
     in `www/index.html` and the `sound` value in `capacitor.config.json` exactly, including the extension.
4. Commit that upload. The workflow runs again, bundles your tone into the APK, and builds a new
   APK with your custom ringtone.
5. **Uninstall the old app from your phone before installing the new APK** — Android locks a
